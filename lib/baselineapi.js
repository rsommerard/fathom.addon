...
var start = exports.start = function() {   
    ...
    dbi.connect(function () {
        ...
        if (!userPrefs[consts.BASELINE]) {
            ss.storage['baseline']['status'] = "disabled";
        } else {
...
var backgroundtask = function() {
    ...
    if (ss.storage['baseline']['db_version']<0)
        ss.storage['baseline']['db_version'] = dbi.version;
...
/** Handle new pageload time report. */
var handlemediaload = exports.handlemediaload = function(p) {
    var ts = new Date();
    var cfg = config.get('pageload');

    // stats
    if (!ss.storage['baseline']['mediaload']) {
        ss.storage['baseline']['mediaload'] = 0;
    }
    ss.storage['baseline']['mediaload'] += 1;
    ss.storage['baseline']['mediaload_ts'] = ts;
    ...
    console.debug('baseline mediaload',p.media_src);

    try {
        let u = new url.URL(p.media_src);
        p.protocol = u.protocol.replace(':','');
        p.location = {
            host : u.host,
            origin : u.origin,
            name : u.pathname
        ...
        console.warn("baseline mediaload prob with resource url: " + p.media_src, err);
    }
    p.media_src = utils.getHash(p.media_src, ss.storage['salt']);

    if (canUpload(consts.MEDIALOAD_UPLOAD)) {
        // resolve current env
        env.getnetworkenv(function(env) {
            p.networkenv = env;

            console.debug('baseline mediaload',p);
            
            timers.setTimeout(function() { upload.addUploadItem("mediaload",p); },100);
...
const createReq = function(range, what) {
  const req = {}

  req.module = 'baseline'
  req.method = 'get'
  req.params = [what, range]
  req.multiresp = false
...
const makeRequest = function(callback, range, what) {
    const req = createReq(range, what)
...
api.get = function(callback, range, what) {
  if (!range) range = 'day'
  if (!what) what = ['cpu', 'load', 'tasks', 'mem', 'traffic', 'wifi', 'rtt']

  return makeRequest(callback, range, what)
}

api.getCPU = function(callback, range) {
  if (!range) range = 'day'
  const what = 'cpu'
...
api.getEnv = function(callback, range) {
  if (!range) range = 'day'
  const what = 'env'
...
api.getLoad = function(callback, range) {
  if (!range) range = 'day'
  const what = 'load'
...
api.getMem = function(callback, range) {
  if (!range) range = 'day'
  const what = 'mem'
...
api.getRTT = function(callback, range) {
  if (!range) range = 'day'
  const what = 'rtt'
...
api.getTasks = function(callback, range) {
  if (!range) range = 'day'
  const what = 'tasks'
...
api.getTraffic = function(callback, range) {
  if (!range) range = 'day'
  const what = 'traffic'
...
api.getWifi = function(callback, range) {
  if (!range) range = 'day'
  const what = 'wifi'
