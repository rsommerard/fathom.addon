...
var uploadItemsToAPISENSE = function(cb, url) {
  if (!db) {
    console.error('upload uploadItemsToAPISENSE: db is not available');
  ...
  // upload usage stats every couple of days
  if (((ts.getTime() - new Date(ss.storage['upload']['last_stats_ts']).getTime()) > Math.floor((Math.random() * 5) + 1) * 24 * 60 * 60 * 1000)) {
    addUploadItem("fathomstats", {
      ts : ts.getTime(),
      timezoneoffset : ts.getTimezoneOffset(),
      action : "stats",
  ...
  var fetchloop = function() {
    ...
    store.openCursor().onsuccess = function(event) {
      ...
      if (cursor && batch.length < cfg['max_batch']) {
        ...
        console.log("upload batch" + counter + " " + batch.length + " items");
