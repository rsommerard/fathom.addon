 ...
 */
var stop = exports.stop = function(reason) {};

/** Resolve the nearest Mlab server. 
 *
 *  FIXME: could cache the value until the current env changes ? 
 */
var getMlabServer = exports.getMlabServer = function(callback) {
    return utils.getJSON(callback, config.get("mlab", "getserverurl"));    
};
 ...
 *   - HTTP HEAD to our API server (use IP address in case google/dns is down)
 */
var isConnected = exports.isConnected = function(callback) {
    ...
    socketapi.exec(function(res, done) {
        ...
        // try a fixed IP in case google/dns was just down (unlikely)
        socketapi.exec(function(res, done) { 
           return callback((!res.error && res.pings.length > 0), true);
        }, {module : 'tools',
            submodule : 'ping',
            method : 'start', 
            params : [config.get('api','ipv4'), pcfg],
            id : 2
        }, apimanifest);
...
var discovery = exports.discovery = function(callback, protocols, timeout, manifest) {
    ...
    var bkptimer = timers.setTimeout(function() {
        ...
        timers.setTimeout(function() { callback(undefined, true); }, 0);
    }, (timeout+3)*1000);
    ...
    var addnode = function(proto, node, pflag) {
        ...
        if (!node && dflag) {
            ...
            timers.setTimeout(function() { callback(undefined, true); }, 0); // signal all done
        } else if (node) {
            if (dflag && bkptimer)
                timers.clearTimeout(bkptimer);
            timers.setTimeout(function() { callback(node, dflag); }, 0); // send node + done flag
        }
    };
    ...
    _.each(protocols, function(p) {
        ...
        case "local":
            // discover local node
            var raw = { proto : p};
            env.getnetworkenv(function(nenv) {
                if (!nenv.error)
                    raw.networkenv = nenv;
        ...
        case "internet":
            env.getnetworkenv(function(raw) {
                if (!raw || raw.error)
                    raw = { };
                raw.proto = p;

                utils.lookupIP(function(res) {
                    if (res && !res.error) {
                        // our public IP (means also that we have internet connection)
                        raw.ip = res;
                        var node = new Node(
                            p,
                            true, // reachable
                            config.get("api", "ipv4"), // use some fixed address that will not appear as other node's address
                            undefined,
                            raw);
                        addnode(p, node, true);                        
                    } else {
                    ...
                    for (var i = 0; i < _.size(rifaces); i++) {
                        ...
                        systemapi.exec(function(res) {
                            ...
                            systemapi.exec(function(res2) {
                                ...
                                params : [(env.getcurrent() ? env.getcurrent().mserver_ip : 'www.google.com'), {
                                  count : 3, 
                                  timeout : timeout, 
                                  interval : 0.5,
                ...
                for (var i = 0; i < _.size(res.result); i++) {
                    ...
                    systemapi.exec(function(pres) {
                        ...
                        if (!pres.error && pres.result.rtt.length > 0) {                            
                            utils.lookupMAC(function(lookupres) {
                                raw.proto = p;
...
var replysearch = function(req) {
    req.result = idblock;
    systemapi.exec(function(res) {
        if (res.error) console.warn("getting hostname failed in replysearch",res);
        var hostname = res.result.split('.')[0];
        req.result.hostname = hostname;
        protoapi.exec(function(res) {
        ...
        if (!discoveryserver) {
            ...
            protoapi.exec(function(id) {
                ...
                protoapi.exec(function(id) {
                    ...
                    timers.setTimeout(function() { callback({}, true); }, 500);
            ...
            console.log("toolsapi there's already a server, doing nothing");
            callback({}, true);
        }
        break;
...
api.iperf.start = function(callback, args) {
    return makeRequest(callback, 'tools', 'iperf', 'start', [args], true)
}

api.iperf.stop = function(callback, id) {
    return makeRequest(callback, 'tools', 'iperf', 'stop', [id], false)
}
...
api.ping.server_start = function(callback, opt, incrementaloutput) {
    return makeRequest(callback, 'tools', 'ping', 'start', [opt], incrementaloutput)
}

api.ping.server_stop = function(callback, id) {
    return makeRequest(callback, 'tools', 'ping', 'stop', [id], false)
}

api.ping.start = function(callback, dst, opt, incrementaloutput) {
    return makeRequest(callback, 'tools', 'ping', 'start', [dst, opt], incrementaloutput)
}
...
api.remoteapi.discovery = function(callback, timeout) {
    return makeRequest(callback, 'tools', 'remoteapi', 'discovery', [timeout], false)
}

api.remoteapi.makereq = function(callback, node, method, params) {
    return makeRequest(callback, 'tools', 'remoteapi', 'makereq', [node, method, params], false)
}

api.remoteapi.start = function(callback) {
    return makeRequest(callback, 'tools', 'remoteapi', 'start', [], false)
}

api.remoteapi.stop = function(callback) {
    return makeRequest(callback, 'tools', 'remoteapi', 'stop', [], false)
}
